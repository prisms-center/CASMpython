        ...
        for input_matrix_type in ['list', 'array']:
            for l_quantum_number in range(4):
                ...
                for spin_polarized in [True, False]:
                    if spin_polarized:
                        test_occupation = OrbitalOccupation(
                            test_matrix_a, test_matrix_b)
                    else:
                        test_occupation = OrbitalOccupation(test_matrix_a)
                self.assertTrue(
                    test_occupation.l_quantum_number == l_quantum_number)
                self.assertTrue(
                    test_occupation.spin_polarized == spin_polarized)
                self.assertTrue(
                    np.array_equal(test_occupation.matrices[0], test_matrix_a))
                if spin_polarized:
                    self.assertTrue(len(test_occupation.matrices) == 2)
                    self.assertTrue(
                        np.array_equal(test_occupation.matrices[1],
                                       test_matrix_b))
                else:
...
class TestCasmVaspReadOccupationMatrix(CasmVaspTestCase):
    ...
    def test_run(self):
        ...
        for case in self.cases:
            input_outcar = join(self.classdir, 'input_data',
                                case['input_data'], 'OUTCAR')
            ...
            for site in case['site_occupation_matrices']:
                for spin_channel, occupation_matrix in enumerate(
                        site['occupation_matrices']):
                    self.assertTrue(
                        test_outcar.orbital_occupations[site['site_index']].
                        matrices[spin_channel].tolist() == occupation_matrix)
...
class TestCasmVaspWriteOccupationMatrix(CasmVaspTestCase):
    ...
    def test_run(self):
        ...
        for case in self.cases:
            occupations = {}
            for site in case['site_occupation_matrices']:
                occupations[site['site_index']] = OrbitalOccupation(
                    *tuple(site['occupation_matrices']))
            output_filename = join(self.classdir, 'output_data',
                                   case['output_data'], 'OCCMATRIX')
            ...
            with open(output_filename, 'r') as output:
                with open(
                        join(self.classdir, 'input_data', case['input_data'],
                             'OCCMATRIX'), 'r') as expected_output:
