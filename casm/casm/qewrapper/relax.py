...
class Relax(object):
    ...
    def __init__(self, configdir=None, auto=True, sort=True):
        ...
        setfile = self.casm_directories.settings_path_crawl(
            "relax.json", self.configname, self.casm_settings.default_clex)
        ...
        if not "infilename" in self.settings:
            print(
                "WARNING: No input file specified in relax.json using default infilename of std.in"
            )
            self.settings["infilename"] = "std.in"
        if not "outfilename" in self.settings:
            print(
                "WARNING: No output file specified in relax.json using default outfilename of std.out"
            )
    ...
    def setup(self):
        ...
        qefiles = qe_input_file_names(self.casm_directories, self.configname,
        ...
        for s in self.settings["extra_input_files"]:
            extra_input_files.append(
                self.casm_directories.settings_path_crawl(
                    s, self.configname, self.casm_settings.default_clex))
        ...
        if self.settings["initial"]:
            extra_input_files += [
                self.casm_directories.settings_path_crawl(
                    self.settings["initial"], self.configname,
                    self.casm_settings.default_clex)
            ]
            if extra_input_files[-1] is None:
                raise quantumespresso.QuantumEspressoError(
                    "Relax.setup failed. No initial  Infile " +
                    self.settings["initial"] + " found in CASM project.")
        if self.settings["final"]:
            extra_input_files += [
                self.casm_directories.settings_path_crawl(
                    self.settings["final"], self.configname,
                    self.casm_settings.default_clex)
            ]
            if extra_input_files[-1] is None:
                raise quantumespresso.QuantumEspressoError(
                    "Relax.setup failed. No final Infile " +
                    self.settings["final"] + " found in CASM project.")

        sys.stdout.flush()

        quantumespresso.qeio.write_quantum_espresso_input(
            self.calcdir, infilename, super_poscarfile, speciesfile, self.sort,
            extra_input_files, self.settings["strict_kpoints"])
    ...
    def run_settings(self):
        ...
        if settings["run_limit"] == None or settings[
    ...
    def run(self):
        ...
        if status == "not_converging":
            ...
            try:
                os.makedirs(
                    self.casm_directories.configuration_calc_settings_dir(
                        self.casm_settings.default_clex))
            except:
                pass
            settingsfile = os.path.join(
                self.casm_directories.configuration_calc_settings_dir(
                    self.casm_settings.default_clex), "relax.json")
            ...
            self.report_status("failed", "unknown")
            raise QEWrapperError(
                "Quantum Espresso relaxation complete with unexpected status: '"
    ...
    def finalize(self):
        ...
        if self.is_converged():
            ...
            speciesfile = self.casm_directories.settings_path_crawl(
                "SPECIES", self.configname, self.casm_settings.default_clex)
    ...
    def is_converged(self):
        ...
        for i in range(len(relaxation.rundir)):
            try:
                qrun = quantumespresso.qeio.QErun(
        ...
        qrun = quantumespresso.qeio.QErun(
            os.path.join(self.calcdir, "run.final", outfilename))
        if not qrun.elec_conv:
            print(
