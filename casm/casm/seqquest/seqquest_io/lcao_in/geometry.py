...
class Geometry(dict):  #pylint: disable=too-few-public-methods
    ...
    __gmethod_keys = ("gmethod", )

    __gmethod_values = ("BROYDEN", "ASD", "DMDAT", "DAMPED", "STEEPEST", None)
    ...
    __all_keys = set(__gmethod_keys + __atom_list_list_keys +
                     __atom_three_list_keys + __vgfix_keys + __int_num_keys +
                     __float_num_keys)
                ...
                for key in self.atom_list_list_keys:
                    if re.search(r"^\s*" + key, line, re.IGNORECASE):
                        if self[key] is None:
                            self[key] = [[
                                int(x)
                                for x in stream.readline().strip().split()
                            ]]
                        else:
                            self[key].append([
                                int(x)
