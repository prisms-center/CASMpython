...
input_example = {
    ...
    "series": [{
        ...
        "to_query": ["configname", "basis_deformation", "lattice_deformation"],
        "tooltips": ["configname", "basis_deformation", "lattice_deformation"]
    }]
}
...
class PlotRankplotCommand(casm.plotting.PlotTypeCommand):
    ...
    def plot(cls, doc, args):
        ...
        for index, series in enumerate(input['series']):
            series['self'] = casm.plotting.RankPlot(data=data,
                                                    index=index,
                                                    **series)
