...
def main():
    ...
    if casm_settings == None:
        raise casm.qewrapper.QEWrapperError(
            "Not in a CASM project. The file '.casm' directory was not found.")

    casm_directories = casm.project.DirectoryStructure()

    calcdir = casm_directories.calctype_dir(configname,
                                            casm_settings.default_clex)

    print("  Reading relax.json settings file")
    sys.stdout.flush()
    setfile = casm_directories.settings_path_crawl("relax.json", configname,
                                                   casm_settings.default_clex)
    ...
    try:
        ...
        print((
            "Unable to report properties for directory %s. Please verify that it contains a completed Quantum Espresso calculation with name %s"
            % (qedir, outfilename)))
        raise
