...
class PlottingData(object):
    ...
    def selection(self,
        ...
        if sel_path not in self.data_[proj.path]['selections']:
            sel = casm.project.Selection(self.project(proj.path), sel_path,
                                         **kwargs)
...
class TapAction(object):
  ...
  """
    ...
    Arguments:
        ...
        data: PlottingData instance
    """
        self.tap_indicator_src = bokeh.models.ColumnDataSource(data={
            "x": [0],
            "project": ['None'],
            "selection": ['None']
        })
        self.js_callback = bokeh.models.CustomJS(
            args={'src': self.tap_indicator_src},
            code="""
      var data = src.data;
      data['x'][0] = data['x'][0] + 1;
    ...
    def __call__(self, attrname, old, new):
        ...
        sel = self.data.selection(proj_path,
                                  sel_path,
                                  proj_kwargs={"verbose": False})
    ...
    def tool(self):
        ...
        return bokeh.models.TapTool(callback=self.js_callback,
...
def _hull_style(input, default):
    for val in [
            'marker', 'hover_color', 'hover_alpha', 'selected',
...
rank_select_cutoff_style = {"color": "red", "line_width": 1, "alpha": 1.0}
...
def scatter_series_style(index, input):
    selected_color = [
        "blue", "red", "green", "black", "cyan", "magenta", "yellow", "orange"
    ]
    unselected_color = [
    ...
    if 'unselected' not in input:
        input['unselected'] = copy.deepcopy(
            default_scatter_style['unselected'])
        ...
        for key in ['radii', 'line_width', 'line_alpha', 'fill_alpha']:
            if key not in input['unselected']:
                input['unselected'][key] = default_scatter_style['unselected'][
                    key]
...
def update_dft_hull_glyphs(sel, style, selected=None):
  ...
  """
    ...
    for value in ['color', 'radii']:
        cmap = dict({
            True: style['selected'][value],
            False: style['unselected'][value]
        })
        set_src_data(sel.src,
                     value,
                     list(map(lambda x: cmap[x], selected)),
                     force=True)
    ...
    if on_dft_hull is not None:
        for value in ['line_color', 'line_width', 'line_alpha']:
            cmap = dict({
                True: style['on_hull'][value],
                False: style['off_hull'][value]
            })
            set_src_data(sel.src,
                         value,
                         list(map(lambda x: cmap[x], on_dft_hull)),
                         force=True)
  ...
  """
    ...
    if on_clex_hull is not None:
        for value in ['line_color', 'line_width', 'line_alpha']:
            ...
            set_src_data(sel.src,
                         'clex_' + value,
                         list(map(lambda x: cmap[x], on_clex_hull)),
                         force=True)
...
def update_scatter_glyphs(sel, style, id, selected=None):
    ...
    for value in [
            'color', 'radii', 'line_color', 'line_width', 'line_alpha',
            'fill_alpha'
    ]:
        cmap = dict({
            True: style['selected'][value],
  ...
  """
    ...
    """
        ...
        if prim.n_independent_compositions != 1:
            ...
            print("n_independent_compositions:",
                  prim.n_independent_compositions)
            raise Exception(
                "Currently ConvexHullPlot only works for binary alloys")

        if selection is None:
            selection = "MASTER"
        self.sel = self.data.selection(project,
                                       selection,
                                       proj_kwargs={"verbose": False})

        if hull_selection is None:
            hull_selection = selection
        self.hull_sel = self.data.selection(project,
                                            hull_selection,
                                            proj_kwargs={"verbose": False})

        self.hull_sel_calculated = casm.project.Selection(
            self.sel.proj, self.hull_sel.path + ".calculated")
        self.dft_hull_sel = casm.project.Selection(
            self.sel.proj, self.hull_sel.path + ".dft_hull")
        ...
        self.clex_style = casm.plotting.clex_hull_style(
            copy.deepcopy(clex_style))
    ...
    def plot(self, fig=None, tap_action=None):
        ...
        self.r_dft = getattr(fig,
                             style['marker'])(self.x,
                                              ...
                                              legend="dft")
        self.r_dft_hull_line = fig.line(
            self.x,
            self.y,
            source=self.calc_hull_src,
            line_color=style['on_hull']['line_color'],
            line_width=style['hull_line_width'],
            line_dash=style['hull_line_dash'],
            legend="dft")

        # clex
        style = self.clex_style
        self.r_clex = getattr(fig, style['marker'])(
            self.x,
            self.y_clex,
            source=self.sel.src,
            size='clex_radii',
            fill_color='clex_color',
            fill_alpha=style['fill_alpha'],
            line_color='clex_line_color',
            line_width='clex_line_width',
            line_alpha='clex_line_alpha',
            hover_alpha=style['hover_alpha'],
            hover_color=style['hover_color'],
            legend="clex")
        self.r_clex_hull_line = fig.line(
            self.x,
            self.y_clex,
            source=self.clex_hull_src,
            line_color=style['on_hull']['line_color'],
            line_width=style['hull_line_width'],
            line_dash=style['hull_line_dash'],
            legend="clex")
        ...
        # clex of dft hull
        style = self.clex_style
        self.r_clex_of_dft_hull_line = fig.line(
            self.x,
            self.y_clex,
            source=self.clex_of_dft_hull_src,
            line_color=style['on_hull']['line_color'],
            line_width=style['hull_line_width'],
        ...
        self.renderers = [
            self.r_dft, self.r_dft_hull_line, self.r_clex,
        ...
        if tap_action is not None:
            tap_action.add_callback(self.sel, view_on_tap,
                                    [self.r_dft, self.r_clex])

        # hover over a point to see 'configname', 'Ef', and 'comp(a)'
        tooltips = [("configname", "@configname")]

        if self.tooltips is not None:
            # tooltips for energy
            if self.Ef_per_atom:
                tooltips.append(
                    ("Ef_per_atom", "@formation_energy_per_atom{1.1111}"))
                tooltips.append(
                    ("clex_Ef_per_atom",
                     "@{clex(formation_energy,per_species)}{1.1111}"))
                tooltips.append(
                    ("hull_dist_per_atom",
                     "@{" + hull_dist_per_atom(self.hull_sel_calculated.path) +
                     "}{1.1111}"))
                tooltips.append(
                    ("clex_hull_dist_per_atom",
                     "@{" + clex_hull_dist_per_atom(self.hull_sel.path) +
                     "}{1.1111}"))
                tooltips.append(
                    ("clex_of_dft_hull_dist_per_atom",
                     "@{" + clex_hull_dist_per_atom(self.dft_hull_sel.path) +
                     "}{1.1111}"))
            else:
                tooltips.append(("Ef", "@formation_energy{1.1111}"))
                tooltips.append(
                    ("clex_Ef", "@{clex(formation_energy)}{1.1111}"))
                tooltips.append(
                    ("hull_dist", "@{" +
                     hull_dist(self.hull_sel_calculated.path) + "}{1.1111}"))
                tooltips.append(
                    ("clex_hull_dist",
                     "@{" + clex_hull_dist(self.hull_sel.path) + "}{1.1111}"))
                tooltips.append(
                    ("clex_of_dft_hull_dist", "@{" +
                     clex_hull_dist(self.dft_hull_sel.path) + "}{1.1111}"))
            ...
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=tooltips,
                                       renderers=[self.r_dft, self.r_clex]))
        else:
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=None,
                                       renderers=[self.r_dft, self.r_clex]))
    ...
    """
        ...
        on_hull = list(
            map(lambda x: abs(x) < self.hull_tol,
                sel.data.loc[_selected(sel), hull_dist_label]))
    ...
    def query(self):
        ...
        self.hull_sel_calculated.save(data=self.hull_sel.data[
            self.hull_sel.data.loc[:, 'is_calculated'] == True],
                                      force=force)
        ...
        self.sel.query([
            'is_calculated', self.x_all, self.y, self.y_clex,
        ...
        # find hull points
        self._set_on_hull(self.sel, 'on_dft_hull',
                          hull_dist_per_atom(self.hull_sel_calculated.path))
        self._set_on_hull(self.sel, 'on_clex_hull',
                          clex_hull_dist_per_atom(self.hull_sel.path))

        # create dft hull selection
        self.dft_hull_sel.save(
            data=self.sel.data[self.sel.data.loc[:, 'on_dft_hull'] == True],
            force=force)
        ...
        self._set_on_hull(self.sel, 'on_clex_of_dft_hull',
                          clex_hull_dist_per_atom(self.dft_hull_sel.path))
        ...
        self.calc_hull_src = bokeh.models.ColumnDataSource(
            data=self.sort_dft_hull_line())

        # source for clex hull data
        self.clex_hull_src = bokeh.models.ColumnDataSource(
            data=self.sort_clex_hull_line())

        # source for clex of dft hull data
        self.clex_of_dft_hull_src = bokeh.models.ColumnDataSource(
            data=self.sort_clex_of_dft_hull_line())
...
#    slef.update_clex_of_dft_hull_line()
    ...
    """
        ...
        return sel.data.loc[on_hull,
                            [self.x, self.y, self.y_clex]].sort_values(
                                [self.x])
    ...
    def sort_clex_of_dft_hull_line(self):
        ...
        self.sorted_clex_of_dft_hull = self._sort_hull_line(
            self.sel, 'on_clex_of_dft_hull')
...
class Scatter(object):
  ...
  """
    ...
    def plot(self, fig=None, tap_action=None):
        ...
        if tap_action is not None:
            tap_action.add_callback(self.sel, casm.plotting.view_on_tap,
                                    [self.r])
        ...
        if self.tooltips is not None:
            ...
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=tooltips, renderers=[self.r]))
        else:
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=None, renderers=[self.r]))
  ...
  """
    ...
    def query(self):
        ...
        if 'to_be_selected' not in self.sel.data.columns:
            self.sel.data.loc[:,
                              'to_be_selected'] = self.sel.data.loc[:,
                                                                    'selected']
        ...
        if 'density' not in self.hist_kwargs.keys(
        ) or self.hist_kwargs['density'] == False:
...
class RankPlot(object):
  ...
  """
    ...
    def plot(self, fig=None, tap_action=None):
        ...
        self.r_stem = fig.segment(self.rank_id,
                                  0,
                                  self.rank_id,
                                  self.score_id,
                                  source=self.sel.src,
        ...
        self.r_circ = fig.circle(self.rank_id,
                                 self.score_id,
                                 source=self.sel.src,
                                 size='radii.' + self.series_name,
                                 fill_color='color.' + self.series_name,
        ...
        if self.tooltips is not None:
            ...
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=tooltips,
                                       renderers=[self.r_circ]))
        else:
            fig.add_tools(
                bokeh.models.HoverTool(tooltips=None, renderers=[self.r_circ]))
    ...
    """
        if self.scoring_query is not None:
            try:
                self.sel.data.loc[:, self.
                                  score_id] = self.sel.data.loc[:, self.
                                                                scoring_query]
            ...
            try:
                module = imp.load_module(self.scoring_module, f, filename,
                                         description)
                self.sel.data.loc[:, self.score_id] = getattr(
                    module, self.scoring_function)(self.sel)
            finally:
                if f:
                    f.close()

        # convert to numeric (NaN)
        self.sel.data.loc[:, self.score_id] = pandas.to_numeric(
            self.sel.data[self.score_id], errors='coerce')

        # add 'score' to src, as self.score_id
        add_src_data(self.sel,
                     self.score_id,
                     self.sel.data.loc[:, self.score_id],
                     force=True)

        # rank
        self.sel.data.loc[
            self.sel.data.sort_values([self.score_id, 'configname']).index,
            self.rank_id] = range(self.sel.data.shape[0])

        # add 'rank' to src as self.rank_id
        add_src_data(self.sel,
                     self.rank_id,
                     self.sel.data.loc[:, self.rank_id],
                     force=True)
  ...
  """
    ...
    """
        ...
        if mode not in ['set', 'set_on', 'set_off']:
            raise ValueError(
                "mode: " + mode +
                " not allowed. Expected 'set', 'set_on', or 'set_off'")
        self.mode = mode
        ...
        _hplot = bk_layouts.row(self.input['cutoff'].widget, self.select_mode,
                                self.select_action)
        self.layout = bk_layouts.column(self.p,
                                        _hplot,
                                        self.msg.widget,
                                        width=self.p.plot_width)
    ...
    def _plot(self):
        ...
        self.p_ = bokeh.plotting.Figure(plot_width=800,
                                        plot_height=400,
                                        tools=_tools)
        p_stem = self.p_.segment(self.rank_id,
                                 0,
                                 self.rank_id,
                                 self.score_id,
                                 source=self.sel.src,
        ...
        p_circ = self.p_.circle(self.rank_id,
                                self.score_id,
                                source=self.sel.src,
                                size='radii',
                                fill_color='color',
        ...
        if self.cutoff_line_src is None:
            self.cutoff_line_src = bokeh.models.ColumnDataSource(
                data={
                    ...
                    'y':
                    [self.input['cutoff'].value, self.input['cutoff'].value]
                })
        ...
        p_click = bokeh.models.CustomJS(args={
            'src': self.cutoff_line_src,
            'loc': self.mouse_location_src
        },
                                        code="""
      var data = src.data;
      ...
      """)
        ...
        p_line = self.p_.line(x='x',
                              y='y',
                              source=self.cutoff_line_src,
                              line_width=style['cutoff_line_width'],
                              line_alpha=style['cutoff_alpha'],
                              color=style['cutoff_color'])

        self.p_.add_tools(
            bokeh.models.HoverTool(tooltips=self.tooltips, renderers=[p_circ]))
    ...
    """
        ...
        try:
            # score
            self.df = pandas.DataFrame(self.scoring(self.sel),
                                       columns=['score'])
        ...
        # rank
        self.df.loc[self.df.sort_values(['score']).index,
                    'rank'] = range(self.df.shape[0])
    ...
    """
        ### mouse location
        self.mouse_location_src = bokeh.models.ColumnDataSource(data={
            'x': [0.0],
            'y': [0.0]
        })
        self.mouse = bokeh.models.HoverTool(
            ...
            callback=bokeh.models.CustomJS(
                args={'src': self.mouse_location_src},
                code="""
        var data = src.data;
        ...
        self.select_mode = bokeh.models.Select(
            value="set", options=["set", "set_on", "set_off"])
        ...
        def select_action_f(attrname, old, new):
            if new == "Revert":
                ...
                update_glyphs(self.sel,
                              selected=self.sel.data['to_be_selected'])
                self.msg.value = "Reverted to last applied selection"
            elif new == "Apply":
                self.sel.data.loc[:,
                                  'selected'] = self.sel.data.loc[:,
                                                                  'to_be_selected']
                self.sel.update_selection()
                update_glyphs(self.sel,
                              selected=self.sel.data['to_be_selected'])
    ...
    """
        ...
        self.sel.data.loc[:, 'to_be_selected'] = self.df.apply(f,
                                                               axis='columns')
        set_src_data(self.cutoff_line_src,
                     'y',
                     [self.input['cutoff'].value, self.input['cutoff'].value],
                     force=True)
        self.sel.update_selection()
...
class WeightSelect(object):
    ...
    """
        ...
        if not os.path.exists(input_filename):
            ...
            with open(self.input_filename, 'wb') as file:
                file.write(
                    six.u(json.dumps(self.fit_input,
                                     indent=2)).encode('utf-8'))
        else:
            self.fit_input = json.loads(
                open(self.input_filename, 'rb').read().decode('utf-8'))
    ...
    def _save(self):
        try:
            ...
            if self.select_method.value == ["wEref"]:
                self.fit_input["weight"]["kwargs"]["Eref"] = self.input[
                    'Eref'].value
            with open(self.input_filename, 'wb') as file:
                file.write(
                    six.u(json.dumps(self.fit_input,
    ...
    def _set_value(self):
        ...
        self.sel.data.loc[:, self.
                          wvalue_id] = self.sel.data.loc[:, 'formation_energy']
        add_src_data(self.sel,
                     self.wvalue_id,
                     self.sel.data.loc[:, self.wvalue_id],
                     force=True)
    ...
    def _set_wvalue(self):
        ...
        if self.select_method.value == "wHullDist":
            w = casm.learn.tools.wHullDist(
                ...
                self.input['A'].value, self.input['B'].value,
                self.input['kT'].value * 0.001)
        elif self.select_method.value == "wEmin":
            w = casm.learn.tools.wEmin(
                self.sel.data['formation_energy'].values[np.where(
                    self._selected)], self.input['A'].value,
                self.input['B'].value, self.input['kT'].value * 0.001)
        ...
        add_src_data(self.sel,
                     'weight',
                     self.sel.data.loc[:, 'weight'],
                     force=True)

        # update data (will update hullplot scatter points, but not convex hull line)
        self.sel.data.loc[self._selected,
                          self.wvalue_id] = casm.learn.tools.set_sample_weight(
                              w,
                              y=self.sel.data.loc[
                                  self._selected,
                                  'formation_energy'].values)[0]
        self.sel.data.loc[self._unselected,
                          self.wvalue_id] = self.sel.data.loc[
                              self._unselected, 'formation_energy']
        add_src_data(self.sel,
                     self.wvalue_id,
                     self.sel.data.loc[:, self.wvalue_id],
                     force=True)
            ...
            if self.select_method.value in ["wEmin", "wEref"]:
                self.ref_line_src = bokeh.models.ColumnDataSource(
                    data={
                        ...
                        'y':
                        [self.input['Eref'].value, self.input['Eref'].value]
                    })
                self.ref_line = self.hullplot.p.line('x',
                                                     'y',
                                                     source=self.ref_line_src,
                                                     line_color='red',
                                                     line_width=1.0)
        if self.ref_line is not None:
            if self.select_method.value == "wEref":
                set_src_data(self.ref_line_src,
                             'x', [self.x_min, self.x_max],
                             force=True)
                set_src_data(
                    self.ref_line_src,
                    'y', [self.input['Eref'].value, self.input['Eref'].value],
                    force=True)
            elif self.select_method.value == "wEmin":
                set_src_data(self.ref_line_src,
                             'x', [self.x_min, self.x_max],
    ...
    def _update_selection(self):
        ...
        self.hull_dist_values = self.sel.data.loc[:,
                                                  hull_dist(self.sel.path
                                                            )].values
    ...
    def _layout(self):
        if self.show_weightplot:
            children = [
                self.hullplot.layout, self.whullplot.layout,
                self.weightplot.layout
            ]
        else:
        ...
        return bk_layouts.column(row,
                                 self.msg.widget,
                                 width=self.whullplot.p.plot_width * 3 + 300)

    def _widget_layout(self):
        if self.select_method.value in ["wHullDist", "wEmin"]:
            return [
                self.select_method.widget, self.input['kT'], self.input['A'],
                self.input['B'], self.select_action
            ]
        else:
            return [
                self.select_method.widget, self.input['kT'], self.input['A'],
                self.input['B'], self.input['Eref'], self.select_action
            ]
...
class ECISelection(object):
    ...
    """
        ...
        self.fit_input = json.loads(
            open(self.input_filename, 'rb').read().decode('utf-8'))
        halloffame_filename = os.path.join(
            cwd, self.fit_input.get("halloffame_filename", "halloffame.pkl"))
        ...
        self.eci = pandas.DataFrame(
            eci, columns=[str(i) for i in range(len(self.hall))])
...
class ECISelect(object):
    ...
    """
        ...
        self.layout = bk_layouts.row(self.cv_rankplot.layout,
                                     self.eci_stemplot)

    def _cv_rankplot(self):
        tooltips = [("Index", "@{index}"), ("CV score", "@{cv}{1.1111}"),
        ...
        add_src_data(self.sel, "estimator_method",
                     self.sel.data["estimator_method"])
        add_src_data(self.sel, "feature_selection_method",
                     self.sel.data["feature_selection_method"])
      ...
      """
            ...
            if len(sel.src.selected['1d']['indices']) == 1:
                print("update!")
                index = sel.src.selected['1d']['indices'][0]
                set_src_data(sel.eci_src,
                             'current',
                             sel.eci_src.data[str(index)],
                             force=True)

        # self, sel, scoring, name="Score", mode='set', tooltips=None, on_tap=view_on_tap)
        self.cv_rankplot = RankPlot(self.sel,
                                    score,
                                    name="CV Score",
                                    tooltips=tooltips,
                                    on_tap=_update_eci_stemplot)
    ...
    def _eci_stemplot(self, index):
        ...
        with open(self.proj.dir.basis(self.proj.settings.default_clex),
                  'rb') as f:
            self.basis = json.loads(f.read().decode('utf-8'))

        set_src_data(sel.eci_src,
                     'index',
                     range(self.sel.eci.shape[0]),
                     force=True)
        set_src_data(sel.eci_src,
                     'branch',
                     [j["orbit"][0] for j in self.basis["cluster_functions"]],
                     force=True)
        set_src_data(sel.eci_src,
                     'mult',
                     [j["mult"] for j in self.basis["cluster_functions"]],
                     force=True)
        set_src_data(sel.eci_src,
                     'max_length', [
                         j["prototype"]["max_length"]
                         for j in self.basis["cluster_functions"]
                     ],
                     force=True)
        set_src_data(sel.eci_src,
                     'min_length', [
                         j["prototype"]["min_length"]
                         for j in self.basis["cluster_functions"]
                     ],
                     force=True)
        colormap = [
        ...
        set_src_data(sel.eci_src,
                     'color',
                     list(
                         map(lambda x: colormap[x % len(colormap)],
                             self.sel.eci_src.data["branch"])),
                     force=True)
        set_src_data(sel.eci_src,
                     'current',
                     self.sel.eci_src.data[str(index)],
                     force=True)

        # TOOLS in bokeh plot
        _tools = ["crosshair,pan,reset,box_zoom,wheel_zoom,save"]

        self.eci_stemplot = bokeh.plotting.Figure(
            plot_width=800,
            plot_height=400,
            tools=_tools,
            y_range=(self.sel.eci.min().min() * 1.1,
                     self.sel.eci.max().max() * 1.1))
        p_stem = self.eci_stemplot.segment("index",
                                           0,
                                           "index",
                                           "current",
                                           source=self.sel.eci_src,
                                           line_color='color',
                                           line_width=1.0)

        p_circ = self.eci_stemplot.circle("index",
                                          "current",
                                          source=self.sel.eci_src,
                                          size=10,
                                          fill_color='color')
        ...
        tooltips = [("Index", "@{index}"), ("ECI", "@{current}{1.1111}"),
                    ("Cluster size", "@{branch}"), ("Multiplicity", "@{mult}"),
                    ("Max length", "@{max_length}"),
                    ("Min length", "@{min_length}")]

        self.eci_stemplot.add_tools(
            bokeh.models.HoverTool(tooltips=tooltips, renderers=[p_circ]))
        self.eci_stemplot.add_tools(
            bokeh.models.BoxSelectTool(renderers=[p_circ]))
        self.eci_stemplot.add_tools(
            bokeh.models.LassoSelectTool(renderers=[p_circ]))
...
class SelectInput(object):
  ...
  """
    ...
    """
        ...
        self.widget = bokeh.models.Select(value=value,
                                          title=title,
                                          options=options)
        self.update = dict()
        self.widget.on_change('value', self.on_change)
  ...
  """
    ...
    """
        ...
        self.widget = bokeh.models.Select(
            value="Options", options=["Options", "Close (without saving)"])
        self.update = []
