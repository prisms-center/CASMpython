...
def job_complete(outfilename, jobdir=None):
    ...
    if (not os.path.isfile(myoutfile)) and (
            not os.path.isfile(myoutfile + ".gz")):
        return False
...
def ionic_steps(outfilename, jobdir=None):
    ...
    try:
        ...
        raise QuantumEspressoIOError(
            "Could not read number of ionic steps from " +
            os.path.join(jobdir, outfilename))
...
def write_quantum_espresso_input(dirpath,
    ...
    print("  Reading K_POINTS reference positions from CELL_PARAMETERS:",
          infilename)
