    ...
    def iter_read(self):
        ...
        if not m:
            while not m:
                ...
                if line == '':
                    raise QErunError(
                        "EOF reach without finding highest occupied level or Fermi energy"
                    )
                line = line.strip()
                m = re.search("highest occupied level.*|.*Fermi energy is.*",
                              line)
        try:
            if line.split()[-1] == "ev":
        ...
        if not m:
            while not m:
                ...
                if line == '':
                    raise QErunError(
                        "EOF reach without finding Forces acting on atoms")
                line = line.strip()
                m = re.search("Forces acting on atoms.*", line)
        forces = []
        f.readline()
        ...
        indexmap = sorted(range(len(names)),
                          key=lambda x: sortednames.index(names[x]))
        rawlist = []
        for site in pos.basis:
