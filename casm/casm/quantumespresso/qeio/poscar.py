...
class Poscar:
    ...
    def _read_lattice_from_infile(self, file):
        ...
        if not m:
            while not m:
                ...
                if line == '':
                    raise PoscarError(
                        "EOF reach without finding CELL_PARAMETERS")
                line = line.strip()
                m = re.match("CELL_PARAMETERS\s*.*", line)
        ...
        if self.scaling.lower() not in ['alat', 'bohr', 'angstrom']:
            raise PoscarError("Read invalid CELL_PARAMETERS units: '" +
                              self.scaling + "'")
        lat = []
        for i in range(3):
            line = file.readline()
        ...
        if self.scaling == "alat":
            ...
            if not m:
                while not m:
                    ...
                    if line == '':
                        raise PoscarError(
                            "EOF reach without finding celldm(1)")
                    line = line.strip()
                    m = re.match(".*celldm.*", line)
            m = re.split("=", m.group(0))
    ...
    def _read_lattice_from_outfile(self, file):
        ...
        if not m:
            while not m:
                ...
                if line == '':
                    raise PoscarError(
                        "EOF reach without finding Begin final coordinates")
                line = line.strip()
                m = re.search("Begin final coordinates.*", line)
        file.readline()
        file.readline()
        ...
        if not (re.match("CELL_PARAMETERS.*", line[0])):
            ...
            if not m:
                while not m:
                    ...
                    if line == '':
                        raise PoscarError(
                            "EOF reach without finding lattice parameter")
                    line = line.strip()
                    m = re.search("lattice parameter.*", line)
            try:
                ...
                raise PoscarError(
                    "Searched top of outfile for alat and was unable to convert to float"
                )
            ...
            if not m:
                while not m:
                    ...
                    if line == '':
                        raise PoscarError(
                            "EOF reach without finding crystal axes:")
                    line = line.strip()
                    m = re.search("crystal axes:.*", line)
            lat = []
            for i in range(3):
                ...
                try:
                    ...
                    raise PoscarError("Could not read lattice vector: '" +
                                      line + "'")
    ...
    def _read_atominfo(self, file):
        ...
        try:
            ...
            raise PoscarError("Could not read number of each atom type: '" +
                              numline + "'")
        if len(self.num_atoms) != len(self.type_atoms):
            raise PoscarError(
                "Atom type and number lists are not the same length: \n'" +
                typeline + "'\n" + "'" + numline + "'")
        self.type_atoms_alias = list(self.type_atoms)
    ...
    def _read_atominfo_from_infile(self, file):
        ...
        if not m:
            while not m:
                ...
                if line == '':
                    raise PoscarError(
                        "EOF reach without finding ATOMIC_POSITIONS")
                line = line.strip()
                m = re.match("ATOMIC_POSITIONS\s*", line)
        line = file.readline().strip()
        atom_list = dict()
        ...
        for i in range(len(self.num_atoms)):
            for j in range(self.num_atoms[i]):
                ...
                try:
                    ...
                    raise PoscarError("Error reading basis coordinate: '" +
                                      line + "'")
                self.basis.append(
                    Site(cart, np.array(pos), SD_FLAG, self.type_atoms[i],
                         self.type_atoms[i]))
        ...
        for i in range(len(self.num_atoms)):
            for j in range(self.num_atoms[i]):
                ...
                if len(words) < 7:
                    ...
                    SD_FLAG = words[4].strip() + ' ' + words[5].strip(
                    ) + ' ' + words[6].strip()
                try:
                    pos = [
                        float(x.strip().replace('d', 'e')) for x in words[1:4]
                    ]
                except ValueError:
                    raise PoscarError("Error reading basis coordinate: '" +
                                      line + "'")
                ...
                self.basis.append(
                    Site(cart, np.array(pos), SD_FLAG, words[0], alias))
        file.seek(0)
    ...
    def _read_basis_atominfo_from_outfile(self, file):
        ...
        while not (re.match("End final coordinates.*", line)):
            ...
            if self.coord_mode.lower() == 'alat':
                ...
                self.basis.append(
                    Site(cart,
                         np.array(pos) * alat, SD_FLAG, words[0], alias))
            else:
                self.basis.append(
                    Site(cart, np.array(pos), SD_FLAG, words[0], alias))
            line = file.readline().strip()
