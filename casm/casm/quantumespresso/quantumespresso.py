...
def continue_job(jobdir, contdir, settings):
    ...
    if os.path.isfile(os.path.join(jobdir, outfilename)) and os.path.isfile(
        ...
        postpos = qeio.Poscar(os.path.join(
            jobdir,
            outfilename))  # see poscar.py regarding run transfer errors
        newinfile = qeio.Infile(os.path.join(contdir, infilename))
        newinfile.rewrite_poscar_info(
            postpos)  # see infile.py regarding run transfer errors
        newinfile.write(os.path.join(contdir, infilename))
        print("Overwrote " + infilename +
              " positions and lattice with positions and lattice from " +
              outfilename)
    else:
...
class FreezeError(object):
    ...
    def error(self, outfilename, line=None, jobdir=None):
        ...
        print("Most recent file output (" + f + "):", most_recent,
              " seconds ago.")
        sys.stdout.flush()
        if t < 300:
            return False
...
class NbandsError(object):
    ...
    def fix(self, err_jobdir, new_jobdir, settings):
        ...
        if len(nbands_line) < 1:
            ...
            print(
                "        Couldn't find any reference to nbands in the Outfile. Continuing without fixing"
            )
        else:
