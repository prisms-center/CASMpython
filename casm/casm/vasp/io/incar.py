...
VASP_TAG_STRING_LIST = ["algo", "prec", "system", "precfock", "lreal", "metagga"]
...
class Incar(object):
    ...
    def __init__(
        self, filename, species=None, poscar=None, sort=True, structure_info=None
    ):
        """Construct an Incar object from 'filename'"""
        self.read(filename, species, poscar, sort, structure_info)

    def read(self, filename, species=None, poscar=None, sort=True, structure_info=None):
        """Read an INCAR file"""
        self.tags = dict()
        try:
    ...
    def update(self, species, poscar, sort=True, structure_info=None):
        ...
        if structure_info is not None and structure_info.atom_properties is not None:
            if "Cmagspin" or "Cunitmagspin" in list(
                structure_info.atom_properties.keys()
            ):
                    ...
                    for alias in sorted(pos.keys()):
                        if key.lower() in (VASP_TAG_SPECF_LIST + VASP_TAG_SPECI_LIST):
                            ...
                            for name in species.keys():
                                if (
                                    species[name].alias == alias
                                    and species[name].write_potcar
                                ):
