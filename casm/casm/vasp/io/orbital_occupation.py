...
class OrbitalOccupation(object):
    ...
    def __init__(self,occupation_matrix_a,occupation_matrix_b=None):
        ...
        if (ma == na and na % 2 == 1):
            ...
            raise OrbitalOccupationError('Occupation matrix a is the wrong shape')
        if occupation_matrix_b is not None:
            ...
            if (mb == nb and nb == na):
                ...
                raise OrbitalOccupationError('Occupation matrices a and b have different shapes')
    ...
    def get_occext_string(self, site_number):
        ...
        def matrix_to_string(matrix):
            return '\n'.join('\t'.join(f'{entry:.4f}' for entry in row) for row in matrix)
...
def write_occupations(filename, occupations):
    ...
    with open(filename, 'w') as output_file:
        ...
        for site_number in occupations.keys():
            output_file.write(occupations[site_number].get_occext_string(site_number+1)) # VASP indexes sites from 1
